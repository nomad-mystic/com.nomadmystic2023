...
export default class HomeAspirationsAnimations extends AnimationMethods {
    ...
    exploreDiscoverCodeAnimations = () => {
        // Animating explore
        const explorePaths = window.document.getElementsByClassName('homePageExploreDiscoverCodeExplorePath');
        this.textPathAnimation(explorePaths);

        // Animating discover
        setTimeout(() => {
            const discoverPaths = window.document.getElementsByClassName('homePageExploreDiscoverCodeDiscoverPath');
            this.textPathAnimation(discoverPaths);
        }, 1000);

        // Animating Code
        setTimeout(() => {
            const codePaths = window.document.getElementsByClassName('homePageExploreDiscoverCodeCodePath');
        ...
        setTimeout(() => {
            // fill Explore first
            const explorePaths = window.document.querySelectorAll('.homePageExploreDiscoverCodeExplorePath');
            this.fillPathAnimation(explorePaths, '1s');

            // fill Discover path after explore finishes
            setTimeout(() => {
                const discoverPaths = window.document.querySelectorAll('.homePageExploreDiscoverCodeDiscoverPath');
                this.fillPathAnimation(discoverPaths, '1s');

                // fill code path after Discover and Explore paths are filled
                setTimeout(() => {

                    const codePaths = window.document.querySelectorAll('.homePageExploreDiscoverCodeCodePath');
